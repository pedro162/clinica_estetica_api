...
use App\Impressao\Com\Tecnick\Barcode\Barcode;
...
class Dabpe extends DaCommon
{
    ...
    public function __construct(
        ...
        $this->xml = $xml;
        $this->aFontTit = ['font' => $this->fontePadrao, 'size' => 9, 'style' => 'B'];
        $this->aFontTex = ['font' => $this->fontePadrao, 'size' => 8, 'style' => ''];
        if (empty($this->xml)) {
    ...
    protected function produtosDABPE($x = 0, $y = 0, $h = 0)
    {
        ...
        $w = $this->wPrint;
        $cont = 0;
        $aFontComp = ['font' => $this->fontePadrao, 'size' => 7, 'style' => 'B'];
        if ($qtdItens > 0) {
    ...
    protected function pagamentosDABPE($x = 0, $y = 0, $h = 0)
    {
        ...
        $aFontPgto = ['font' => $this->fontePadrao, 'size' => 7, 'style' => ''];
        $wBoxEsq = $w * 0.7;
