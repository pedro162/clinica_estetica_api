...
class EloquentTemplateRepository implements TemplateRepositoryInterface
{
    ...
    {
        $template = DB::table('templates')->where('id', '=', (string)$id)->first();
        if ($template) {
            $objTemplate =  new Template();
            $objTemplate->setId(new TemplateId($template->id));
            $objTemplate->setLanguage(new TemplateLanguage($template->language));
            $objTemplate->setTitle(new TemplateTitle($template->title));
            $objTemplate->setBody(new TemplateBody($template->body ?? ''));
            return $objTemplate;
        }
        return null;
    }
